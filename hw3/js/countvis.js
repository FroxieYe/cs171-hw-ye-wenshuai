...
CountVis = function(_parentElement, _data, _metaData, _eventHandler){
    ...
    this.margin = {top: 15, right: 15, bottom: 20, left: 50},
...
CountVis.prototype.updateVis = function(){
    ...
    function brushed(){
        ...
        if (!that.brush.empty()){
            var v1 = d3.min([e[0],e[1]])
            var v2 = d3.max([e[0],e[1]])
            $(that.eventHandler).trigger("selectionChanged", [v1, v2])
        }
        else{
...
CountVis.prototype.addSlider = function(svg){
    ...
    var sliderDragged = function(){
        ...
        var sliderValue = sliderScale.invert(value)+0.1;
        var powerScale = d3.scale.pow().exponent((sliderValue/100+0.1)).range([that.height, 20])
